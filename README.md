...
基于 FastAPI 的本地个人办公工具箱。采用「壳 + 插件」架构：壳永远轻量，工具按需安装、代码按需加载、依赖按需引入，装多少工具都不会臃肿。开箱内置电子书制作、MD转公文Word、抖音下载、B站下载、音频切分工具、图片分辨率工具六个插件域，后续工具即插即用。
...
| 🖼️ 图片分辨率工具 | jpg / png / webp / bmp / tiff / gif（单张 / 多张 / 文件夹 / zip·7z 压缩包，支持拖入整个文件夹） | 图片（保持原格式或 JPG / PNG / WebP，可整包 zip） | 分辨率调整（长边/短边/百分比/宽/高五选一，保比不拉伸，可只缩不放）；图片压缩（按质量或目标体积 KB 二分逼近，可叠加缩放，压完反而更大自动保留原图）；图片增强（2倍放大 + 二次锐化 / 锐化 / 自动对比度 / 降噪）；EXIF 方向自动转正，透明通道转 JPG 铺白底；压缩包内目录结构原样保留，坏图跳过并生成处理报告 |
...
├── requirements.txt   # 本插件的核心依赖清单（首次使用插件时由壳自动安装）
├── requirements-*.txt # 可选：可选依赖组（重依赖只属于个别动作时用），设置页可见、不自动安装
...
- **新增：** 电子书制作「从豆瓣链接一键填充」——粘贴豆瓣书籍链接即可自动填入书名 / 作者 / 出版社 / ISBN / 出版日期 / 简介，并自动下载豆瓣封面图一并填入封面区（多 CDN 主机兜底重试，抓取失败时明确提示而非静默跳过；Markdown → EPUB 与 TXT → EPUB 两个动作均支持）
- **新增：** 电子书制作封面支持本地拖拽 / 点击上传——新增通用「文件型参数」能力（manifest 声明 `type: "file"` 即自动生成拖拽控件，可选 `accept` 限制格式、`allowUrl` 保留粘贴图片链接的备用方式），上传文件以与参数同名的字段提交、服务端落盘后回填路径，插件侧无需改动
- **新增：** 通用表单支持多行文本框——manifest 参数加 `rows` 即渲染为 textarea（内容简介默认 4 行）；置顶「抓取工具条」由动作声明 `douban: true` 触发
- **新增：** API 接口文档左侧目录导航——按「通用约定 / 通用接口 / 执行动作详解 + 各插件」列出目录，点击平滑跳转到对应区块，滚动时自动高亮当前区块，窄屏自动切换为顶部整宽列表
- **修复：** 首页刷新后「API 接口」按钮位置漂移（进入工具再返回才归位）——统一所有回首页路径的布局写法，`margin-top: auto` 重新生效

### v0.3

- **新增：** 图片分辨率工具插件：图片压缩 / 增强 / 分辨率调整三个动作；输入支持单张、多张、整个文件夹（直接拖入）、压缩包（zip / 7z，rar 走可选依赖组），压缩包内目录结构原样保留，结果超过 20 张自动附整包 zip 下载，坏图自动跳过并生成处理报告，本地处理不上传
- **新增：** 分辨率调整：目标长边 / 目标短边 / 缩放百分比 / 指定宽 / 指定高五选一（前两者「以图为准」适合横竖混排批量对齐，后两者「以版面为准」定轴保比），可开关「只缩不放」防小图放大变虚，插值算法可选（默认 LANCZOS）
- **新增：** 图片压缩：按质量百分比或按目标体积（KB，二分逼近）压缩，可叠加缩放；输出保持原格式或转 JPG / WebP；本就高度压缩的小图「压完反而更大」时自动保留原图；重存显式指定质量，杜绝默认 quality=75 的隐性降质
- **新增：** 图片增强：2倍放大（LANCZOS 放大 + 二次锐化收拢软边）/ 锐化（三档强度）/ 自动对比度 / 降噪，四项可自由叠加；EXIF 方向自动转正（手机竖拍图不再转错），透明通道转 JPG 自动铺白底
- **新增：** 所有插件的文件上传动作均支持拖入整个文件夹：前端递归收集文件夹内全部匹配文件（对其它插件的批量上传同样生效）
- **新增：** 设置页「一键卸载」——每个插件（含壳）可一键卸载其全部已装依赖（Python 包 + Chromium/Whisper 等大组件），卸载前确认弹窗列出待移除项
- **新增：** 大文件下载直链——转换结果按文件大小自动选择下载方式，超过 200MB 的文件由浏览器流式下载、不再整体占用内存
- **新增：** API 服务与接口文档——所有工具均可通过 HTTP API 调用（执行动作、轮询进度、取回结果、扫码登录、依赖管理），首页底部新增「API 接口」按钮，点开即见由各插件 manifest 自动生成的完整接口文档与可复制的 curl 示例，装了新插件刷新即见
- **优化：** 按需加载落实到位——插件支持热插拔（拷入插件文件夹后刷新首页即可见，无需重启服务）；「视频转文字」的重依赖拆分为可选依赖组，只下载视频/音频不再被迫安装数 GB 的转写依赖
- **优化：** 项目稳定性整体加固——修复抖音「真实 msToken」静默失效的问题；接口请求与大文件下载增加超时与停滞兜底，浏览器组件下载增加看门狗超时；设置页操作进度丢失后自动收场，不再卡死；登录弹窗改为仅右上角 × 关闭
- **优化：** 安全与文件名规范——下载路由补齐文件名校验、拦截路径穿越；上传文件与结果文件（B站标题、书名等外部文本）统一净化非法字符、同名自动去重；前端动态文本统一 HTML 转义；抖音图文作品的图片不再一律标注为「封面」
- **优化：** 性能与下载体验——任务产物直接搬移而非复制，大文件少写一遍磁盘；首次自动安装插件依赖的进度实时可见，进度弹窗只透出关键日志（下载 / 安装 / 完成），不再因长日志频繁伸缩
- **优化：** 项目整体结构清理——统一重复实现（中文数字转换等）、移除僵尸代码与过期注释；抖音下载精简，转写动作重命名为「视频转文字」并精简描述；设置页视觉细节打磨（卸载按钮红色标识、刷新与卸载按钮右缘对齐、各模块说明列对齐）
